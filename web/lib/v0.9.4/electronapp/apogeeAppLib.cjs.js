...
var apogeeCoreLib_js = require('./apogeeCoreLib.cjs.js');
...
class ModelManager extends FieldObject {
    ...
    load(workspaceManager,json) {
        ...
        if(!modelJson) modelJson = apogeeCoreLib_js.Model.EMPTY_MODEL_JSON;
        if(!componentsJson) componentsJson = ModelManager.EMPTY_MODEL_COMPONENT_JSON;

        //create model
        let model = new apogeeCoreLib_js.Model(workspaceManager.getModelRunContext());
        ...
        let actionResult = apogeeCoreLib_js.doAction(model,loadAction);
...
addcomponent.executeCommand = function(workspaceManager,commandData) { 
    ...
    let actionResult = apogeeCoreLib_js.doAction(model,createAction);
...
deletecomponent.executeCommand = function(workspaceManager,commandData) {
    ...
    var actionResult = apogeeCoreLib_js.doAction(model,actionJson);
    if(!actionResult.actionDone) {
...
futuremodelactioncommand.executeCommand = function(workspaceManager,commandData) {
    ...
    var actionResult = apogeeCoreLib_js.doAction(model,commandData.action);
    if(!actionResult.actionDone) {
...
movecomponent.executeCommand = function(workspaceManager,commandData) {
    ...
    let actionResult = apogeeCoreLib_js.doAction(model,actionData);
    if(!actionResult.actionDone) {
...
savemembercode.executeCommand = function(workspaceManager,commandData) {
    ...
    var actionResult = apogeeCoreLib_js.doAction(model,actionData);
    if(!actionResult.actionDone) {
...
updatecomponent.executeCommand = function(workspaceManager,commandData) {
    ...
    if(memberGenerator.getPropertyUpdateAction) {
        ...
        if(actionData) {
            ...
            actionResult = apogeeCoreLib_js.doAction(model,actionData);
            if(!actionResult.actionDone) {
...
exports.BaseFileAccess = BaseFileAccess;
